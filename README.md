...
| `/shared-review` | Local senior-review reader where a reviewer can paste a Markdown or JSON packet generated by the app. |
...
Hosted private share links are not implemented because this repository has no approved backend/storage pattern. A future PRD is recorded in `docs/superpowers/specs/2026-05-25-hosted-private-share-links-prd.md`.
...
- `src/questionTiming.ts` — lightweight local per-question timing helpers and pacing summaries.
- `src/shareReport.ts` — local-first senior review packet derivation, Markdown rendering, and import parsing.
